...
class CategoryController extends BaseController
{
    ...
    public function listData($langId, $category_id, $type = '', $limit = 12, $userid, $can_add_product){
        if($type == 'vendor' && $can_add_product == 0){
            ...
            $vendorData = Vendor::select('id', 'name', 'banner', 'show_slot', 'order_pre_time', 'order_min_amount', 'vendor_templete_id')->where('status', '!=', $this->field_status)->whereIn('id', $vendor_ids)->with('slot', 'avgRating')->paginate($limit);
