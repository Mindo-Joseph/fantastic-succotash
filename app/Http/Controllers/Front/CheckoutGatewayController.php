...
class CheckoutGatewayController extends FrontController
{
    ...
    public function checkoutSuccessApp(Request $request)
    {
        ...
        if ($response->result->status == 'PAID') {
            $returnUrlParams = '?status=200&gateway=checkout&action=' . $request->payment_form . '&transaction_id=' . $transactionId;
            if($request->payment_form == 'cart'){
            ...
            $returnUrlParams = '?status=0&gateway=checkout&action=' .$request->payment_form;
            if($request->payment_form == 'cart'){
                $order_number = $request->order;
                $order = Order::with(['paymentOption', 'user_vendor', 'vendors:id,order_id,vendor_id'])->where('order_number', $order_number)->first();
                $order_products = OrderProduct::select('id')->where('order_id', $order->id)->get();
